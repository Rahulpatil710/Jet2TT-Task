...
protocol BlogsNetworkProtocol {
    func fetchBlogs(_ page: Int, completion: @escaping((Blogs?, HTTPStatusCode.ResponseType, String?) -> Void))
}
    ...
    func fetchBlogs(_ page: Int, completion: @escaping((Blogs?, HTTPStatusCode.ResponseType, String?) -> Void)) {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            if let status = response as? HTTPURLResponse,
                let statusCode = HTTPStatusCode(rawValue: status.statusCode) {
                ...
                case .success:
                    ...
                    do {
                        ...
                        completion(blogs, .success, nil)
                    } catch let error {
                        completion(nil, .undefined, error.localizedDescription)
                    }
                    
                default:
                    completion(nil, statusCode.responseType, error?.localizedDescription)
                }
            }
        }
        task.resume()
